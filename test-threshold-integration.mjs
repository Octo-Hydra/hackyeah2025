  ...
  try {
    ...
    for (const scenario of testScenarios) {
      ...
      if (decision.affectedRoutes) {
        console.log(`   Affected routes: ${decision.affectedRoutes.join(", ")}`);
      }
    }
    ...
    for (const testCase of journeyTestCases) {
      ...
      console.log(`\n${testCase.name}:`);
      console.log(`   Extracted line IDs: ${lineIds.length > 0 ? lineIds.join(", ") : "[]"}`);
    }
    ...
    console.log(`   Base report count: ${DEFAULT_THRESHOLD_CONFIG.baseReportCount}`);
    console.log(
    ...
    if (sampleUser) {
      ...
      console.log(`   Trust score: ${sampleUser.trustScore || "Not calculated yet"}`);
    ...
    console.log("     - DEFAULT_THRESHOLD_CONFIG (highReputationThreshold, highReputationBonus)");
